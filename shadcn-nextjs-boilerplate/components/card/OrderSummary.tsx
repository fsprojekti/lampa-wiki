...
export default function OrderSummary() {
    const {
        ...
        walletAddress,
        orderDuration,
        selectedNetwork  // ✅ Use selected network from Global Context
    } = useGlobalContext();
    ...
    async function handlePlaceOrder() {
        ...
        if (printerID === "") {
            alert("Please define the printer ID of your order.");
            return;
        }

        try {
            console.log(`🚀 Placing order on ${selectedNetwork}...`);
            const result = await placeOrder(
                selectedNetwork, // ✅ Use the selected network
                walletAddress,
                orderID,
                printerID,
                orderPrice,

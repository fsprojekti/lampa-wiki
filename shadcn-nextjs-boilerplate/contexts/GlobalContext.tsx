...
interface GlobalContextProps {
    ...
    selectedNetwork: "base-mainnet" | "base-testnet" | "arbitrum-sepolia";
...
export function GlobalProvider({ children }: { children: ReactNode }) {
    ...
    const [selectedNetwork, setSelectedNetwork] = useState<"base-mainnet" | "base-testnet" | "arbitrum-sepolia">("base-mainnet");
    ...
    useEffect(() => {
        ...
        console.log(`🔄 Fetching data for wallet: ${walletAddress} on ${selectedNetwork}`);

        async function fetchData() {
            try {
                const owner = await fetchContractOwner(selectedNetwork);
                setContractOwner(owner);

                const printers = await fetchPrinters(selectedNetwork);
                setProductionPrinters(printers);

                const orders = await fetchOrders(selectedNetwork, walletAddress);
                setProductionOrders(orders);

                const ethBalance = await fetchBalanceETH(walletAddress);
                setBalanceETH(ethBalance);

                const erc20Balance = await fetchBalanceERC20(selectedNetwork, walletAddress);
                setBalanceERC20(erc20Balance);

                console.log("✅ Contract data successfully fetched.");
            } catch (error) {
